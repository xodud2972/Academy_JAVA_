...
// 이 객체는 안드로이드에서도 동일하게 쓰인다.
	...
	HttpURLConnection http; // URLConnection의 자식 객체 즉 Http프로토콜에 최적화된 잣힉
	...
	public ImageCollector() {
		...
		try {
			...
			http = (HttpURLConnection)con; // 다운캐스팅! 같은 자료형이므로 형 변환 가능
			http.setRequestMethod("GET"); // GET 방식의 요청
			...
			int data = -1;
			byte[] b = new byte[1024]; // 배열의 크기가 바가지의 크기를 결정한다. 1k byte 씩 담기
			
			while(true) {
				data = is.read();
				if(data==-1)break;
				System.out.println(data);
				fos.write(data); // 1byte 씩 파일에 내려쓰기
			}
