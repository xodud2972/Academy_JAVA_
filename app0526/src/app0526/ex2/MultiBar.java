	...
	public MultiBar() {
		...
		bt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				t1 = new Thread(bar1); // runnable과 thread의 연결
				t2 = new Thread(bar2); // runnable과 thread의 연결
				t3 = new Thread(bar3); // runnable과 thread의 연결
				
				// 이 시점부터 개발자가 start() 메서드를 호출하면 해당 thread의
				//run메서드호출은 runnable 인터페이스를 구현한 객체의 것을 호출한다.
				t1.start();
				t2.start();
				t3.start();
			}
